...
extern char *Com_MD5File(const char *fn, int length, const char *prefix, int prefix_len);
...
/*
	Paul
	Struct/var for keeping track of the amount of ip/ports sending getstatus requests
*/
typedef struct ipTries_s{
	netadr_t		ip;
	unsigned short	port;
	short			tries;
	int				lastReq;
...
extern cvar_t *sv_authclients;

extern cvar_t *sv_allowcl;
