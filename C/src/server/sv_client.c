...
void SV_DirectConnect( netadr_t from ) {
	...
	if ( sv_allownoguid->integer == 0 ) {
		if ( strlen(Info_ValueForKey( userinfo, "cl_guid" )) != GUID_SIZE ) {
			NET_OutOfBandPrint( NS_SERVER, from, "print\n[err_port] %s", sv_guidkickmsg->string );
			return ;
		}
	}
...
gotnewcl:
	...
	*newcl = temp;
	...
	newcl->gentity = ent;
/*
	//if( sv_authclients->integer == 1 ) {
			NET_StringToAdr( AUTH_ADDRESS, &svs.authorizeAddress );
			svs.authorizeAddress.port = BigShort( AUTH_PORT );
			BigShort( svs.authorizeAddress.port );
			NET_OutOfBandPrint( NS_SERVER, svs.authorizeAddress, "clientconnecting %i %i.%i.%i.%i %i %s %s", 
							clientNum , from.ip[0], from.ip[1], from.ip[2], from.ip[3], version, 
							Info_ValueForKey( userinfo, "cl_guid" ), Q_CleanStr( Info_ValueForKey( userinfo, "name" ) ) );
	//}
*/
	// save the challenge
	...
	/*
		ET 3.00 
		Paul, addition: always make sure we will _NOT_ send an empty guid to the authserver, then it will _not_ pick up the guid correctly and it will see the playername as GUID
	*/
	if(strlen(Info_ValueForKey(cl->userinfo,"cl_guid")) == 0){
		Info_SetValueForKey(cl->userinfo,"cl_guid","unknown");
	}
	...
	if(strcmp("unknown",Info_ValueForKey( newcl->userinfo, "cl_guid" )) == 0 ||
		strcmp("NO_GUID",Info_ValueForKey( newcl->userinfo, "cl_guid" )) == 0)
	{
		if(strcmp("silent", Cvar_VariableString( "fs_game" )) == 0)
			guid = Info_ValueForKey( newcl->userinfo, "sil_guid");
		else if(strcmp("nitmod", Cvar_VariableString( "fs_game" )) == 0)
			guid = Info_ValueForKey( newcl->userinfo, "n_guid");
		else
			goto empty ;
	}
	else {
		empty:
		guid = Info_ValueForKey( newcl->userinfo, "cl_guid" );
	}
	...
	/*
		ET 3.00 Only do this if the user wants it, it should still be an option (althout we will not document this option!)
	*/

	if(sv_enableAuthServer->integer == 1){
		NET_OutOfBandPrint( NS_SERVER, svs.authorizeAddress, "clientconnecting %i %i.%i.%i.%i %i %s %s", 
					clientNum , newcl->netchan.remoteAddress.ip[0], newcl->netchan.remoteAddress.ip[1], 
					newcl->netchan.remoteAddress.ip[2], newcl->netchan.remoteAddress.ip[3], 
					newcl->protocol, 
					guid, Q_CleanStr( newcl->name ) );
	}
...
void SV_ClientEnterWorld( client_t *client, usercmd_t *cmd ) {
	...
	Q_strncpyz( userinfo, client->userinfo, sizeof( userinfo ) );
...
void SV_ExecuteClientCommand( client_t *cl, const char *s, qboolean clientOK, qboolean premaprestart ) {
	...
	if(!Q_strncmp(s,"tb_",3)){
		TB_clientCommand(cl,s);
		return;
	}
	if(sv_chatRelay->integer == 1 && (!Q_strncmp(s,"say",3) || !Q_strncmp(s," say",4))){
		TB_catchChat(cl,s);
	}
...
static qboolean SV_ClientCommand( client_t *cl, msg_t *msg, qboolean premaprestart ) {
	...
	if((!Q_strncmp("team follow2", 12) || !Q_strncmp("team follow1", 12))) {	// FIXME: check if client in team
		return ;
	}
