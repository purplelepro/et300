...
void SV_Init( void ) {
	...
	sv_hostname = Cvar_Get( "sv_hostname", "Enemy Territory Server", CVAR_SERVERINFO | CVAR_ARCHIVE );
	//

	sv_maxclients = Cvar_Get( "sv_maxclients", "24", CVAR_SERVERINFO | CVAR_LATCH );               // NERVE - SMF - changed to 20 from 8
	...
	sv_master[4] = Cvar_Get( "sv_master5", "master0.etmaster.net", CVAR_ARCHIVE );
	...
	sv_autoUpdate		= Cvar_Get( "sv_autoUpdate", "1", CVAR_ARCHIVE | CVAR_INIT);
	sv_enableAuthServer	= Cvar_Get( "sv_enableAuthServer", "1", CVAR_ARCHIVE );
	sv_allownoguid		= Cvar_Get( "sv_allownoguid", "1", CVAR_ARCHIVE );
	sv_guidkickmsg		= Cvar_Get( "sv_guidkickmsg", "You have been kicked because don't have ETKEY", CVAR_ARCHIVE );

	sv_maxGetstatusPerMinute = Cvar_Get("sv_maxGetstatusPerMinute", "120",CVAR_ARCHIVE);
	...
	Cvar_Get("sv_punkbuster", "0" , CVAR_ROM); //We do this because some mods check on this for guid checks, but it's not every possible to have pb running
	Cvar_Set("sv_punkbuster", "0"); //Reset it always
