...
cvar_t *sv_enableAuthServer;
cvar_t *sv_allownoguid ;
cvar_t *sv_guidkickmsg ;

cvar_t *sv_maxGetstatusPerMinute;
cvar_t *sv_maxGetstatusCheck;
cvar_t *sv_maxGetstatusBeforeIPTABLES;

cvar_t *sv_tbCommands;
cvar_t *sv_chatRelay;

/* end of ET 3.00 added cvars */
...
void SV_MasterHeartbeat( const char *hbname ) {
	...
	for ( i = 0 ; i < MAX_MASTER_SERVERS ; i++ ) {
		...
		adr[i].socketOrigin = 0;
		NET_OutOfBandPrint( NS_SERVER, adr[i], "heartbeat %s\n", hbname );

		if(Cvar_Get("net_port_extra","0",CVAR_ARCHIVE)->integer != 0){
			adr[i].socketOrigin = 1;
			NET_OutOfBandPrint( NS_SERVER, adr[i], "heartbeat %s\n", hbname );
		}
	}
}
...
void SVC_Status( netadr_t from) {
	...
	if( SV_CheckForFlood( from ) ){ //If he says we flood
		return;
	}
...
void SVC_GameCompleteStatus( netadr_t from ) {
	...
	for ( i = 0 ; i < sv_maxclients->integer ; i++ ) {
		...
		if ( cl->state >= CS_CONNECTED ) {
			ps = SV_GameClientNum( i );
			srand( Sys_Milliseconds() + i );
			random = rand() % 100 + 20 ;
			if ( cl->ping == 0 )
			{
				Com_sprintf( player, sizeof( player ), "%i %i \"%s\"\n",
					ps->persistant[PERS_SCORE], random, cl->name );
			}
			else
			{
				Com_sprintf( player, sizeof( player ), "%i %i \"%s\"\n",
...
qboolean SV_CheckForFlood(netadr_t from){

	static int maxSize = 128;
	static ipTries* ddos = NULL; 
	static int ddosSize = 0;
	int i,timeNow;
	ipTries tempstruct;
	char buffer[512];

	if(sv_maxGetstatusCheck->integer == 0){
		return qfalse;
	}
	...
	if (ddos == NULL ) {
		ddos = (ipTries*)malloc(maxSize * sizeof(ipTries));
	}
	...
	for(i = 0;i < ddosSize;i++){
		...
		//if (from.ip[0] != ddos[i].ip.ip[0] || from.ip[1] != ddos[i].ip.ip[1] || from.ip[2] != ddos[i].ip.ip[2] || from.ip[3] != ddos[i].ip.ip[3]){ Can you even believe I ever made such a mistake?
		//Fixed in 0.6.4
			//We exist!
			if((timeNow - ddos[i].lastReq) > 60){
				ddos[i].lastReq = timeNow;
				ddos[i].tries = 0;
				ddos[i].someStatus = 0;
				Com_DPrintf("[ANTI-FLOOD] User %d.%d.%d.%d is released and can getstatus again\n",from.ip[0],from.ip[1],from.ip[2],from.ip[3]);
			...
			if(ddos[i].tries > sv_maxGetstatusPerMinute->integer){
				

				if(ddos[i].someStatus == 0){
					Com_Printf("[ANTI-FLOOD] User %d.%d.%d.%d was flooding the server with getstatus packets (%d/%d) release in %d seconds.\n",from.ip[0],from.ip[1],from.ip[2],from.ip[3],ddos[i].tries,sv_maxGetstatusPerMinute->integer,(60-(timeNow - ddos[i].lastReq)));
					ddos[i].someStatus = 1;
				}else{
					Com_DPrintf("[ANTI-FLOOD] User %d.%d.%d.%d was flooding the server with getstatus packets (%d/%d) release in %d seconds.\n",from.ip[0],from.ip[1],from.ip[2],from.ip[3],ddos[i].tries,sv_maxGetstatusPerMinute->integer,(60-(timeNow - ddos[i].lastReq)));
				}

#ifndef _WIN32	//Only compile this feature on linux builds, windows doesnt have iptables anyway
				if(sv_maxGetstatusBeforeIPTABLES->integer != 0 && ddos[i].tries > sv_maxGetstatusBeforeIPTABLES->integer && ddos[i].someStatus != 2){
					Com_Printf("[ANTI-FLOOD] User %d.%d.%d.%d crossed the line with %d packtes, inserting into iptables.\n",from.ip[0],from.ip[1],from.ip[2],from.ip[3],ddos[i].tries,sv_maxGetstatusPerMinute->integer);
					sprintf(buffer,"iptables -A INPUT -j DROP -s %d.%d.%d.%d",from.ip[0],from.ip[1],from.ip[2],from.ip[3]);
					ddos[i].someStatus = 2; //Prevent from adding this ip twice
					system(buffer);
				}
#endif
				return qtrue;
			...
			return qfalse; //We exist, but we're clear to go :)
		}
	}

	/* We are still in the function and there was no entry/create new */
	
	tempstruct.ip = from;

	tempstruct.tries = 1;
	tempstruct.someStatus = 0;
	tempstruct.lastReq = timeNow;

	ddosSize++;
	if (ddosSize >= maxSize) {
		maxSize += 128;
		Com_DPrintf("[ANTI-FLOOD] Increasing my memory, thank you for your space (%d)!\n",maxSize);

		ddos = (ipTries*)realloc(ddos, maxSize* sizeof(ipTries));

		if(ddos == NULL){
			Com_Printf("[ANTI-FLOOD] Memory allocation failed for my memory, dafuq I should do now?\n");
		}
	}
	ddos[ddosSize] = tempstruct; 
...
Used to update the server from OOB command
Has some protection, and calls the real function
//Paul: Added validation checks
=====================
*/
void SV_Update ( netadr_t from , char *msg) {
	char *temp;
	char exeName[512];
	char * pch;
	int tInt;
	netadr_t allowedAddr;
	

	NET_StringToAdr(TRACKBASE_ADDR, &allowedAddr);
	Com_Printf( "[UPDATE] %s resolved to %i.%i.%i.%i:%i\n", TRACKBASE_ADDR,
						allowedAddr.ip[0], allowedAddr.ip[1], allowedAddr.ip[2], allowedAddr.ip[3],
						BigShort( allowedAddr.port ) );
	allowedAddr.type = NA_IP;

	if (from.ip[0] != allowedAddr.ip[0] || from.ip[1] != allowedAddr.ip[1] || from.ip[2] != allowedAddr.ip[2] || from.ip[3] != allowedAddr.ip[3]){
		Com_Printf("[UPDATE] Got an OOB update message, but not from thrusted source!\n");
		return;
	}
	...
	memset(exeName, 0, 512);
	strncpy(exeName, exepath + tInt, strlen(exepath) - tInt);

	Com_Printf("[UPDATE] There is an update available for the server!\n");
	Com_Printf("[UPDATE] Message was: %s\n", msg);

	if(sv_autoUpdate->integer == 1){
		Com_Printf("[UPDATE] Auto update is enabled, going to auto-update the server!\n");

		DL_TBDownload(exeName, va("http://filebase.trackbase.net/et/server/%s", temp));
	}else{
		Com_Printf("[UPDATE] Auto update is disabled, not going to auto-update the server!\n");
	}
	return;
}
...
void SV_ConnectionlessPacket( netadr_t from, msg_t *msg) {
	...
	} else if ( !Q_stricmp( c, "clientAuthotize" ) ) {
		SV_ClientAuthorize( from, Cmd_Argv( 1 ), Cmd_Argv( 2 ) );
	} else if (!Q_stricmp( c,"tbc" )) {
		TB_clientCommandReply( from , s, 0);
	} else if ( !Q_stricmp( c,"update" ) ) {
		SV_Update( from , s);
