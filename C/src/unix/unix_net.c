...
void NET_OpenIP( void ) {
	...
	for(j = 0;j<2;j++){
		...
		if(ip_socket[j] == 0){
			Com_Printf("Couldn't allocate IP port" );
			return;
		}
	}
}
...
void NET_Sleep( int msec ) {
	...
	/*if ( !ip_socket[0] || !com_dedicated->integer ) {
	...
	if(ip_socket[0] != 0){
		FD_SET( ip_socket[0], &fdset ); // network socket
		highestfd = ip_socket[0];
	}
	if(ip_socket[1] != 0){
		FD_SET( ip_socket[1], &fdset ); // network socket
