...
static SOCKET ip_socket[2] = {INVALID_SOCKET};
...
qboolean Sys_GetPacket( netadr_t *net_from, msg_t *net_message,int sockIndex) {
	int ret;
	...
	net_from->socketOrigin = 0;
	if(sockIndex == 1){
		net_from->socketOrigin = 1;
		
	}

	for ( protocol = 0 ; protocol < 2 ; protocol++ ) {
		if ( protocol == 0 ) {
			net_socket = ip_socket[sockIndex];
		} else {
		...
		if ( ret == SOCKET_ERROR ) {
			...
			Com_Printf( "NET_GetPacket [%i]: %s\n", sockIndex, NET_ErrorString() );
			continue;
		}

		if ( net_socket == ip_socket[sockIndex] ) {
			memset( ( (struct sockaddr_in *)&from )->sin_zero, 0, 8 );
		}

		if ( usingSocks && net_socket == ip_socket[sockIndex] && memcmp( &from, &socksRelayAddr, fromlen ) == 0 ) {
...
void Sys_SendPacket( int length, const void *data, netadr_t to ) {
	...
	if(to.socketOrigin == 1 && ip_socket[1] == 0){
		printf("^1[ERROR] ^7Packet is send off-socket [%d]!\n",to.socketOrigin);
		printf("---- Packet details ----\nIP:%s\nPacket:%s\n=== Please forward this message to the ET 3.00 developers ===\n",NET_AdrToString(to),data);
		return;
	}

	if(to.socketOrigin != 0 && to.socketOrigin != 1){
		to.socketOrigin = 0; //If we somehow have _no_ socket use the main one anyway
		printf("^1[ERROR] ^7socketOrigin out of range [%d]!",to.socketOrigin);
	}

	if ( to.type == NA_BROADCAST ) {
		net_socket = ip_socket[to.socketOrigin];
...
void NET_OpenIP( void ) {
	...
	port_extra = Cvar_Get( "net_port_extra", va( "%i", 0 ), CVAR_LATCH )->integer;
	...
	for(j = 0;j<2;j++){
		...
		for ( i = 0 ; i < 10 ; i++ ) {
			ip_socket[j] = NET_IPSocket( ip->string, port + i );
			if ( ip_socket[j] ) {
				if(j == 0){
					Cvar_SetValue( "net_port", port + i );
				}
				if(j == 1){
					Cvar_SetValue("net_port_extra", port + i);
				}
...
void NET_Config( qboolean enableNetworking ) {
	...
	if ( stop ) {
		if ( ip_socket[0] && ip_socket[0] != INVALID_SOCKET ) {
			closesocket( ip_socket[0] );
			ip_socket[0] = 0;
		}

		if ( ip_socket[1] && ip_socket[1] != INVALID_SOCKET ) {
			closesocket( ip_socket[1] );
			ip_socket[1] = 0;
		}
